...
// Copyright (c) 2014 The Peonycredit developers
...
const string strMessageMagic = "Peonycredit Signed Message:\n";
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    else if(nHeight<=97920)
    {
        nSubsidy=200*COIN;
    }
    else if(nHeight<=146880)
    {
        nSubsidy=400*COIN;
    }
    else if(nHeight<=195840)
    {
        nSubsidy=800*COIN;
    }
    else if(nHeight<=293760)
    {
        nSubsidy=1600*COIN;
    }
    else if(nHeight<=391680)
    {
        nSubsidy=3200*COIN;
    }
	...
	else if(nHeight<=881280)
    {
        nSubsidy=25600*COIN;
    }
	else if(nHeight<=1077120)
    {
        nSubsidy=51200*COIN;
    }
	else if(nHeight<=1566720)
    {
        nSubsidy=102400*COIN;
    }
	else if(nHeight<=1860480)
    {
        nSubsidy=51200*COIN;
    }
	else if(nHeight<=2154240)
    {
        nSubsidy=25600*COIN;
    }
	...
	else if(nHeight<=2741760)
    {
        nSubsidy=6400*COIN;
    }
	else if(nHeight<=3035520)
    {
        nSubsidy=3200*COIN;
    }
	else if(nHeight<=3329280)
    {
        nSubsidy=1600*COIN;
    }
	else if(nHeight<=3623040)
    {
        nSubsidy=800*COIN;
    }
	...
	else if(nHeight<=4210560)
    {
        nSubsidy=200*COIN;
    }
	else if(nHeight<=4504320)
    {
        nSubsidy=100*COIN;
    }
	else if(nHeight<=4798080)
    {
        nSubsidy=50*COIN;
    }
	else if(nHeight<=48046720)
    {
        nSubsidy=25*COIN;
    }
    else
...
static const int64 nInterval = 240; // retargets blocks

static const int64 nAveragingInterval = 240; // 240 blocks
static const int64 nAveragingTargetTimespan = nAveragingInterval * nTargetSpacing;

static const int64 nMaxAdjustDown = 2; // 2% adjustment down
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PeonycreditMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PeonycreditMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("Peonycredit miner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    ...
    {
        printf("Peonycredit miner terminated\n");
        throw;
    }
}
