...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: peonycredit.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: peonycreditd.pid)") + "\n";
    ...
    strUsage += "  -sf=<algo>           " + _("Mining algorithm: sha256d, scrypt") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Algo
    std::string strAlgo = GetArg("-sf", "sha256d");
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Peonycredit is probably already running."), strDataDir.c_str()));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    printf("Peonycredit version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
    ...
    if (fDaemon)
        fprintf(stdout, "Peonycredit server starting\n");
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
        {
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of Peonycredit") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart Peonycredit to complete") << "\n";
